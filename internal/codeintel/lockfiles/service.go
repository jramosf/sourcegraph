...
func (s *Service) StreamDependencies(ctx context.Context, repo api.RepoName, rev string, cb func(reposource.PackageDependency) error) (err error) {
	...
	paths2 := []string{}
	for _, pathspec := range lockfilePathspecs {
		paths2 = append(paths2, string(pathspec))
	}

	paths, err := s.gitSvc.LsFiles(ctx, repo, api.CommitID(rev), paths2...)
	if err != nil {
		return err
	}
	...
	paths3 := []gitserver.Pathspec{}
	for _, path := range paths {
		paths3 = append(paths3, gitserver.Pathspec(path))
	}
